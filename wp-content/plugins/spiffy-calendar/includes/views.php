...
<tr><td colspan="2" class="cat-key-cell"><strong>'.esc_html($spiffy_calendar->current_options['category_plural']).'</strong></td></tr>';
		...
		foreach($this->categories as $cat_detail) {
			if ( ($cat_list == '') || (in_array($cat_detail->category_id, $filtered_cats))) {
				$output .= '<tr class="'.$row_classes.'" data-category="'.$cat_detail->category_id.'"><td style="background-color:' . esc_html($cat_detail->category_colour) . '; " class="cat-key-cell"></td>
<td class="cat-key-cell" data-category="'.$cat_detail->category_id.'">&nbsp;'.$cat_detail->category_name.'</td></tr>';
			}
		}
		$output .= '</table>';		
	...
	*/
	function draw_event_popup($event, $type)
	{
		...
		$details .= '<span class="spiffy-title">' . esc_html(stripslashes($event->event_title)) . '</span>';
		...
		$details .= '<span class="event-title" ' . $style_popup . '>' . esc_html(stripslashes($event->event_title)) . '</span>';
...
ORDER BY event_id";
	...
	*/
	function todays_events($cat_list = '', $event_limit = '', $style = '', $show_date = 'false', $none_found = '', $title = ' ', $num_columns = '')
	{
		...
		$event_count = 0;
		foreach($events as $event) {
			$output .= '<li class="spiffy-event-details spiffy-'.$style.' spiffy-num'.$num_columns.'">'.$this->draw_event($event, $style, $current_timestamp).'</li>';
			$event_count ++;
	...
	function upcoming_events($cat_list = '', $event_limit = '', $style = '', $none_found = '', $title = ' ', $num_columns = '')
	{
		...
		while ($day_count < $future_days+1)	{
			...
			foreach($events as $event) {
				...
				$output .= '<li class="spiffy-event-details spiffy-'.$style.' spiffy-num'.$num_columns.'">'.$this->draw_event($event, $style, $this_timestamp).'</li>';
				$event_count ++;
				if (($event_limit != '') && ($event_count >= $event_limit)) break;
			}
			if ( (count($events) != 0) && ($style != 'Columns') ) {
